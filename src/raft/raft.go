...
type RequestVoteReply struct {
	...
	VoteState   VoteState // 投票状态--这个是论文里面没有的，后面看看可不可以去掉
}
...
func (rf *Raft) sendRequestVote(server int, args *RequestVoteArgs, reply *RequestVoteReply, voteNums *int) bool {
	...
	// DPrintf("[	sendRequestVote(%v) ] : send a election to %v\n", rf.me, server)
	...
	case Normal, Voted:
		...
		DPrintf("[	sendRequestVote-func-rf(%v)		] check vote %v %v\n", rf.me, reply.VoteGranted, reply.Term)
		...
		// 票数超过一半
		DPrintf("[	sendRequestVote-func-rf(%v)		] check voteNums %v\n", rf.me, *voteNums)
		if *voteNums >= (len(rf.peers)/2)+1 {
			*voteNums = 0
			...
			DPrintf("[	sendRequestVote-func-rf(%v)		] be a leader\n", rf.me)
		}
	case Killed:
		return false
	}
...
// 5. 建立心跳, 每个node根据自己的角色决定时间到了之后的行为，leader进行同步，follower/Candidate 参与选举
func (rf *Raft) ticker() {
	for rf.killed() == false {
		...
		case Candidate:
			...
			rf.timer.Reset(rf.overtime)
			// 对自身以外的节点进行选举,这里的rf.peer应该是在test的时候添加的
			for i := 0; i < len(rf.peers); i++ {
				...
				// DPrintf("[	ticker(%v) ] : send a election to %v\n", rf.me, i)
